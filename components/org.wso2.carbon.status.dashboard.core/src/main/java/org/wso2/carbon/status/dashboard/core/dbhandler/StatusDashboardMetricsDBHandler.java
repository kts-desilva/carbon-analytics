...
public class StatusDashboardMetricsDBHandler {
    ...
    public StatusDashboardMetricsDBHandler() {
        ...
        if (dataSource != null) {
            try {
                ...
                selectWorkerThroughputQuery = metricsQueryManager.getQuery(SQLConstants.SELECT_WORKER_THROUGHPUT_QUERY);
                selectWorkerAggregatedMetricsQuery = metricsQueryManager
                        .getQuery(SQLConstants.SELECT_WORKER_AGGREGATE_METRICS_QUERY);
                selectWorkerAggregatedThroughputQuery = metricsQueryManager
                        .getQuery(SQLConstants.SELECT_WORKER_AGGREGATE_THROUGHPUT_QUERY);
                selectAppComponentList = metricsQueryManager.getQuery(SQLConstants.SELECT_COMPONENT_LIST);
                selectAppComponentHistory = metricsQueryManager.getQuery(SQLConstants.SELECT_COMPONENT_METRICS_HISTORY);
                recordSelectAggregatedAppMetricsQuery = metricsQueryManager
                        .getQuery(SQLConstants.SELECT_APP_AGG_METRICS_HISTORY);
                selectAppComponentAggregatedHistory = metricsQueryManager
                        .getQuery(SQLConstants.SELECT_COMPONENT_AGG_METRICS_HISTORY);
    ...
    /**
     * Method which can be used to clear up and ephemeral SQL connectivity
     * artifacts.
     *
     ...
     * @return
     */
    public List<List<Object>> selectAppComponentsHistory(String workerId, String appName, long timeInterval,
            long currentTimeMilli, String metricsType, String componentType, String componentId, boolean isCount) {
        ...
        String resolvedQuery = resolvedSelectWorkerMetricsHistoryQuery
                .replace(SQLConstants.PLACEHOLDER_BEGIN_TIME, QUESTION_MARK).replace(PLACEHOLDER_NAME, QUESTION_MARK)
     ...
     * @return
     */
    public List<List<Object>> selectAppComponentsAggHistory(String workerId, String appName, long timeInterval,
            long currentTimeMilli, String metricsType, String componentType, String componentId, boolean isCount) {
        ...
        String componentName = APP_NAME_PREFIX + appName + PACKAGE_NAME_SEPARATOR + "Siddhi" + PACKAGE_NAME_SEPARATOR
                + componentType + PACKAGE_NAME_SEPARATOR + componentId + PACKAGE_NAME_SEPARATOR + metricsType;
        String resolvedSelectWorkerMetricsHistoryQuery = resolveTableName(selectAppComponentAggregatedHistory,
                tableName);
        String resolvedQuery = resolvedSelectWorkerMetricsHistoryQuery
                .replace(SQLConstants.PLACEHOLDER_BEGIN_TIME, QUESTION_MARK).replace(PLACEHOLDER_NAME, QUESTION_MARK)
                .replace(PLACEHOLDER_WORKER_ID, QUESTION_MARK)
        ...
        if (!isCount) {
            ...
            resolvedQuery = resolvedQuery.replace(PLACEHOLDER_AGGREGATION_COMPONENT_COLOUM,
                    tableAggColumn.get(tableName));
            Object[] parameters = new Object[] { workerId, componentName + PERCENTAGE_MARK,
                    currentTimeMilli - timeInterval, currentTimeMilli };
            return select(resolvedQuery, tableColumn.get(tableName), tableName, parameters);
        } else {
     ...
     */
    public Map<String, List<String>> selectAppComponentsList(String workerId, String appName, int timeInterval,
        ...
        for (String tableName : METRICS_TABLE_NAMES) {
            ...
            String resolvedQuery = resolvedSelectWorkerMetricsQuery
                    ...
                    .replace(SQLConstants.PLACEHOLDER_CURRENT_TIME, QUESTION_MARK);
            Object[] parameters = new Object[] { workerId,
                    APP_NAME_PREFIX + appName + PACKAGE_NAME_SEPARATOR + PERCENTAGE_MARK,
                    currentTimeMilli - timeInterval, currentTimeMilli };
    ...
    public List selectComponentsLastMetric(String carbonId, String appName, Map<String, List<String>> components,
        ...
        for (Map.Entry componentEntry : components.entrySet()) {
            ...
            for (String tableEntry : componentsTableList) {
                ...
                String resolvedSelectWorkerRecentMetricsQuery = resolveTableName(selectAppComponentHistory, tableEntry);
                ...
                Object[] recentQueryParameters = new Object[] { carbonId,
                        String.format(STRING_TEMPLATE + STRING_TEMPLATE + STRING_TEMPLATE, componentEntry.getKey(),
                                PACKAGE_NAME_SEPARATOR, PERCENTAGE_MARK),
                        currentTimeMilli - timeInterval, currentTimeMilli };
                ...
                List<List<Object>> selectionRecent = select(resolvedRecentQuery, columnListString, tableEntry,
                        recentQueryParameters);
                List<Object> selection = new ArrayList<>();
                if (selectionRecent.size() > 0) {
                    selection = selectionRecent.get(selectionRecent.size() - 1);
                }
                String[] componentElements = ((String) componentEntry.getKey())
                        .replace(APP_NAME_PREFIX + appName + PACKAGE_NAME_SEPARATOR + "Siddhi" + PACKAGE_NAME_SEPARATOR,
                                "")
                        .split("\\.", 2);
                ...
                if ((selection != null) && (!selection.isEmpty())) {
                    ...
                    if ((!("Streams".equalsIgnoreCase(componentElements[0])))
                        ...
                        attribute = new Attribute(columnList[1],
                                humanReadableByteCount((double) selection.get(1), true));
                    } else {
                        attribute = new Attribute(columnList[1],
                                NumberFormat.getNumberInstance().format(selection.get(1)));
                    }
                    ...
                    if (selectAppComponentsHistory(carbonId, appName, timeInterval, System.currentTimeMillis(),
                            METRICS_TYPE_THROUGHPUT, componentElements[0], componentElements[1], true).size() > 0) {

                    }
                    componentMetrics.setTotalEvents(getEventsCount(componentElements[0], componentElements[1], carbonId,
                            appName, timeInterval));
    ...
    private long getEventsCount(String componentType, String componentId, String carbonId, String appName,
        ...
        case "trigger":
        case "streams": {
            List<List<Object>> metrics = selectAppComponentsHistory(carbonId, appName, timeInterval,
                    System.currentTimeMillis(), METRICS_TYPE_THROUGHPUT, componentType, componentId, true);
            if (metrics.size() > 0) {
        ...
        case "storequeries": {
            List<List<Object>> metrics = selectAppComponentsHistory(carbonId, appName, timeInterval,
                    System.currentTimeMillis(), METRICS_TYPE_LATENCY, componentType, componentId, true);
            if (metrics.size() > 0) {
                return (long) metrics.get(0).get(1);
            } else {
                return 0;
        ...
        case "tables": {
            List<List<Object>> metricsLatency = selectAppComponentsHistory(carbonId, appName, timeInterval,
                    System.currentTimeMillis(), METRICS_TYPE_LATENCY, componentType, componentId, true);
            List<List<Object>> metricsThroughput = selectAppComponentsHistory(carbonId, appName, timeInterval,
                    System.currentTimeMillis(), METRICS_TYPE_THROUGHPUT, componentType, componentId, true);
        ...
        default: {
            throw new StatusDashboardRuntimeException("Metrics type '" + componentType.toLowerCase() + "' not valid.");
        }
    ...
    /**
     * Convert memory bytes into human readable format. unit bytes value is decided
     * by based on SI format or not. Ref:
     ...
     */
    public List selectAppOverallMetrics(String metricsType, String workerId, long timeInterval, String appName,
            long currentTime) {
        switch (metricsType) {
        case "memory": {
            String resolvedQueryTable = selectAppMetricsQuery
                    .replace(SQLConstants.PLACEHOLDER_COLUMNS, EXPR_SUM_FROM_STRING)
                    .replace(SQLConstants.PLACEHOLDER_BEGIN_TIME, QUESTION_MARK)
                    .replace(PLACEHOLDER_NAME, QUESTION_MARK).replace(PLACEHOLDER_WORKER_ID, QUESTION_MARK)
                    .replace(SQLConstants.PLACEHOLDER_CURRENT_TIME, QUESTION_MARK)
            ...
            Object[] parameters = new Object[] { workerId,
                    APP_NAME_PREFIX + appName + PACKAGE_NAME_SEPARATOR + PERCENTAGE_MARK, currentTime - timeInterval,
                    currentTime };
            return selectAppMemory(resolvedQueryTable, METRICS_TABLE_METRIC_GAUGE, parameters, COLUMN_TIMESTAMP);
        }
        case "throughput": {
            String resolvedQueryTable = selectAppMetricsQuery
            ...
            Object[] parameters = new Object[] { workerId,
                    APP_NAME_PREFIX + appName + PACKAGE_NAME_SEPARATOR + PERCENTAGE_MARK, currentTime - timeInterval,
                    currentTime };
            return select(resolvedQueryTable, COLUMN_TIMESTAMP + SEPARATOR + COLUMN_M1_RATE, METRICS_TABLE_METRIC_METER,
                    parameters);
        }
        case "latency": {
            String resolvedQueryTable = selectAppMetricsQuery
            ...
            Object[] parameters = new Object[] { workerId,
                    APP_NAME_PREFIX + appName + PACKAGE_NAME_SEPARATOR + PERCENTAGE_MARK, currentTime - timeInterval,
                    currentTime };
            return select(resolvedQueryTable, COLUMN_TIMESTAMP + SEPARATOR + COLUMN_M1_RATE, METRICS_TABLE_METRIC_TIMER,
                    parameters);
        }
        default: {
            logger.error(
                    "Invalid parameters type: " + removeCRLFCharacters(workerId) + ":" + removeCRLFCharacters(appName));
            return null;
        }
        }
    }
     ...
     * @return selected list of metrics.
     */
    public List selectAppAggOverallMetrics(String metricsType, String workerId, long timeInterval, String appName,
            long currentTime) {
        long aggregationTime = DBTableUtils.getAggregation(timeInterval);
        switch (metricsType) {
        case "memory": {
            ...
            String resolvedQueryTable = recordSelectAggregatedAppMetricsQuery
                    ...
                    .replace(PLACEHOLDER_NAME, QUESTION_MARK).replace(PLACEHOLDER_WORKER_ID, QUESTION_MARK)
                    .replace(SQLConstants.PLACEHOLDER_CURRENT_TIME, QUESTION_MARK)
                    .replace(PLACEHOLDER_RESULT, COLUMN_VALUE).replace(PLACEHOLDER_TABLE_NAME, tableName)
                    .replace(PLACEHOLDER_AGGREGATION_TIME, Long.toString(aggregationTime));
            Object[] parameters = new Object[] { workerId,
                    APP_NAME_PREFIX + appName + PACKAGE_NAME_SEPARATOR + PERCENTAGE_MARK, currentTime - timeInterval,
                    currentTime };
     ...
     * @return List<List<Object>> of metrics data because charts needed in that
     *         format
     */
    public List selectWorkerMetrics(String workerId, long timeInterval, String metricTypeName, long currentTime) {
     ...
     *         format
     */
    public List selectWorkerAggregatedMetrics(String workerId, long timeInterval, String metricTypeName,
            long currentTime) {
        long aggregationTime = DBTableUtils.getAggregation(timeInterval);
        String resolvedSelectWorkerMetricsQuery = resolveTableName(selectWorkerAggregatedMetricsQuery,
                METRICS_TABLE_METRIC_GAUGE);
        String resolvedQuery = resolvedSelectWorkerMetricsQuery
                .replace(SQLConstants.PLACEHOLDER_BEGIN_TIME, QUESTION_MARK).replace(PLACEHOLDER_NAME, QUESTION_MARK)
                .replace(PLACEHOLDER_WORKER_ID, QUESTION_MARK)
    ...
    public List selectWorkerThroughput(String workerId, long timeInterval, long currentTime) {
        ...
        return select(resolvedQuery, COLUMN_TIMESTAMP + SEPARATOR + COLUMN_M1_RATE, METRICS_TABLE_METRIC_METER,
                parameters);
    }
    ...
    public List selectWorkerAggregatedThroughput(String workerId, long timeInterval, long currentTime) {
        ...
        String resolvedQuery = resolvedSelectWorkerThroughputQuery
                ...
                .replace(PLACEHOLDER_AGGREGATION_TIME, Long.toString(aggregationTime));
        Object[] parameters = new Object[] { workerId, currentTime - timeInterval, currentTime };
        return select(resolvedQuery, COLUMN_AGG_TIMESTAMP + SEPARATOR + COLUMN_M1_RATE, METRICS_TABLE_METRIC_METER,
                parameters);
    }
    ...
    private List<List<Object>> selectAppMemory(String query, String tableName, Object[] parameters,
        ...
        try {
            ...
            while (rs.next()) {
                row = new ArrayList<>();
                row.add(DBTableUtils.getInstance().fetchData(rs, timesStampLabel,
                        attributesTypeMap.get(timesStampLabel), metricsQueryManager));
            ...
            throw new RDBMSTableException("Error retrieving records from table '" + tableName + "': " + e.getMessage()
                    + " in " + DATASOURCE_ID, e);
        } finally {
            try {
                if (rs != null) {
            ...
            while (rs.next()) {
                ...
                for (String columnLabel : columnLabels) {
                    if (columnLabel.equalsIgnoreCase(COLUMN_VALUE)) {
                        row.add(Double.valueOf((String) DBTableUtils.getInstance().fetchData(rs, columnLabel,
                                attributesTypeMap.get(columnLabel), metricsQueryManager)));
                    } else {
                        row.add(DBTableUtils.getInstance().fetchData(rs, columnLabel,
                                attributesTypeMap.get(columnLabel), metricsQueryManager));
                    }
                }
                tuple.add(row);
    ...
    private List<List<Object>> selectGauge(String query, boolean isAggregated, Object[] parameters) {
        ...
        try {
            ...
            while (rs.next()) {
                row = new ArrayList<>();
                row.add(DBTableUtils.getInstance().fetchData(rs, timestampCol, attributesTypeMap.get(COLUMN_TIMESTAMP),
                        metricsQueryManager));
                row.add(Double.valueOf((String) DBTableUtils.getInstance().fetchData(rs, COLUMN_VALUE,
                        attributesTypeMap.get(COLUMN_VALUE), metricsQueryManager)));
                tuple.add(row);
